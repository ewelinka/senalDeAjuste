...
{   
  ...
  void initWords(){
    ...
    for (int i = 0; i < splittedMsg.length; i++) {
      ...
      if(w.length() > 4 && random(100)<20) canGrow = true;
  ...
  class Word {
    ...
    void growIfYouCan(){
      if(canGrow) {
        ...
        if(!(xWordStart < 2 || xWordEnd > width-2 || topWord < 2)) fontSize= min(fontSize+2,60);
      }
    }
