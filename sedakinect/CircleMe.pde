...
{   
  ...
  void drawScene(){
    ...
    if (inBetween<totalTweens && circleNumber+1<models) {
      ...
      if(dancers.hasDancers()){
        ...
        doSimpleOutline(hybridCircle, pos);
  ...
  void doSimpleOutline(float[] myCircle, PVector p) {
    masterX=masterX+3.0;
    //doDrawCircle(int(masterX), int(masterY), myCircle);
    doDrawCircle(p.x , p.y, myCircle);
  ...
  void doDrawCircle(float xPos, float yPos, float[] ptRad) { 
    pg.beginDraw();  
    pg.strokeWeight(0.2);
    pg.fill(255, 4);
    pg.pushMatrix();
